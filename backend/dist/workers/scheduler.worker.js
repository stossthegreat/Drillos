...
exports.schedulerQueue = new bullmq_1.Queue(QUEUE_NAME, { connection: redis_1.redis });
/**
 * === Environment toggles ===
 */
...
async function bootstrapSchedulers() {
    ...
    await exports.schedulerQueue.add("scan-alarms", {}, { repeat: { every: 60_000 }, ...base });
    await exports.schedulerQueue.add("ensure-daily-briefs", {}, { repeat: { every: 60 * 60_000 }, ...base });
    await exports.schedulerQueue.add("auto-nudges-hourly", {}, { repeat: { every: 60 * 60_000 }, ...base });
    await exports.schedulerQueue.add("ensure-evening-debriefs", {}, { repeat: { every: 60 * 60_000 }, ...base });
}
/**
 * === Worker ===
 */
new bullmq_1.Worker(QUEUE_NAME, async (job) => {
    switch (job.name) {
        case "scan-alarms":
...
async function scanDueAlarms() {
    const now = new Date();
    const due = await db_1.prisma.alarm.findMany({ where: { enabled: true, nextRun: { lte: now } } });
    let processed = 0;
    for (const alarm of due) {
        try {
            ...
            if (await canUseLLM(user.id, plan)) {
                text = await ai_service_1.aiService.generateMentorReply(user.id, mentor, `Alarm fired: ${alarm.label}`);
                await incLLM(user.id);
            }
            let audioUrl = null;
            if (text && (await canUseTTS(user.id, plan, text.length))) {
                audioUrl = await voice_service_1.voiceService.ttsToUrl(user.id, text, mentor).catch(() => null);
            ...
            if (plan === "PRO" || FREE_NOTIFICATIONS_ENABLED) {
                await notifications_service_1.notificationsService.send(alarm.userId, alarm.label, text.slice(0, 180));
            }
            processed++;
        }
        catch (e) {
...
async function runDailyBrief(userId) {
    ...
    if (await canUseLLM(user.id, plan)) {
        text = await ai_service_1.aiService.generateMorningBrief(userId, mentor);
        await incLLM(user.id);
    }
    let audioUrl = null;
    if (await canUseTTS(user.id, plan, text.length)) {
        audioUrl = await voice_service_1.voiceService.ttsToUrl(userId, text, mentor).catch(() => null);
        await incTTS(user.id, text.length);
    }
    await db_1.prisma.event.create({ data: { userId, type: "morning_brief", payload: { text, audioUrl } } });
    await notifications_service_1.notificationsService.send(userId, "Morning Brief", text.slice(0, 180));
    return { ok: true };
}
async function autoNudgesHourly() {
    const users = await db_1.prisma.user.findMany({ select: { id: true, plan: true } });
    for (const u of users) {
        const plan = u.plan || "FREE";
        if (!(await canUseLLM(u.id, plan)))
            continue;
        const mentor = (await db_1.prisma.user.findUnique({ where: { id: u.id } }))?.mentorId || "marcus";
        const result = await nudges_service_1.nudgesService.generateNudges(u.id);
        await incLLM(u.id);
        if (!result.success || !result.nudges || result.nudges.length === 0)
            continue;
        const nudge = result.nudges[0];
        await notifications_service_1.notificationsService.send(u.id, "Nudge", nudge.message.slice(0, 180));
    }
    return { ok: true };
}
async function ensureEveningDebriefJobs() {
    ...
    for (const u of users) {
        const tz = u.tz || "Europe/London";
        await exports.schedulerQueue.add("evening-debrief", { userId: u.id }, {
...
async function runEveningDebrief(userId) {
    const user = await db_1.prisma.user.findUnique({ where: { id: userId } });
    if (!user)
        return { ok: false, reason: "no-user" };
    const plan = user.plan || "FREE";
    ...
    if (await canUseLLM(user.id, plan)) {
        text = await ai_service_1.aiService.generateEveningDebrief(userId, mentor);
        await incLLM(user.id);
    }
    let audioUrl = null;
    if (await canUseTTS(user.id, plan, text.length)) {
    ...
    await db_1.prisma.event.create({ data: { userId, type: "evening_debrief", payload: { text, audioUrl } } });
    await notifications_service_1.notificationsService.send(userId, "Evening Debrief", text.slice(0, 180));
    return { ok: true };
}
/* =========================
   Helper Utilities
   ========================= */
function llmKey(userId) {
    const d = new Date().toISOString().split("T")[0];
    return `llm:count:${userId}:${d}`;
}
function ttsKey(userId) {
    const d = new Date().toISOString().split("T")[0];
...
async function incLLM(userId) {
    const ttl = secondsUntilMidnight();
    await redis_1.redis.multi().incr(llmKey(userId)).expire(llmKey(userId), ttl).exec();
}
async function incTTS(userId, chars) {
    const ttl = secondsUntilMidnight();
    await redis_1.redis.multi().incrby(ttsKey(userId), chars).expire(ttsKey(userId), ttl).exec();
}
async function canUseLLM(userId, plan) {
    if (!LLM_ENABLED)
        return false;
    const cap = plan === "PRO" ? LLM_DAILY_MSG_CAP_PRO : LLM_DAILY_MSG_CAP_FREE;
    const used = Number((await redis_1.redis.get(llmKey(userId))) || 0);
    return used < cap;
}
async function canUseTTS(userId, plan, chars) {
    if (!TTS_ENABLED)
        return false;
    const cap = plan === "PRO" ? TTS_DAILY_CHAR_CAP_PRO : TTS_DAILY_CHAR_CAP_FREE;
    const used = Number((await redis_1.redis.get(ttsKey(userId))) || 0);
